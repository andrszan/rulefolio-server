...
CREATE TABLE public.identity_accounts (
    ...
    CONSTRAINT ck_identity_account_status CHECK (((status)::text = ANY ((ARRAY['pending_activation'::character varying, 'active'::character varying, 'disabled'::character varying])::text[])))
);
...
CREATE TABLE public.identity_one_time_credentials (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT ck_identity_token_purpose CHECK (((purpose)::text = ANY ((ARRAY['account_activation'::character varying, 'password_recovery'::character varying, 'workspace_invitation'::character varying])::text[]))),
    CONSTRAINT ck_identity_token_status CHECK (((status)::text = ANY ((ARRAY['active'::character varying, 'consumed'::character varying, 'revoked'::character varying])::text[])))
);
...
CREATE TABLE public.identity_recovery_request_jobs (
    ...
    CONSTRAINT ck_identity_recovery_job_status CHECK (((status)::text = ANY ((ARRAY['pending'::character varying, 'processing'::character varying, 'completed'::character varying, 'failed'::character varying])::text[])))
);
...
CREATE TABLE public.mail_outbox (
    ...
    CONSTRAINT ck_mail_outbox_status CHECK (((status)::text = ANY ((ARRAY['pending'::character varying, 'sending'::character varying, 'accepted'::character varying, 'failed'::character varying, 'unknown'::character varying, 'cancelled'::character varying, 'suppressed'::character varying])::text[])))
);
...
CREATE TABLE public.work_accesses (
    ...
    CONSTRAINT ck_work_access_role CHECK (((role)::text = ANY ((ARRAY['maintainer'::character varying, 'organizer'::character varying, 'collaborator'::character varying])::text[])))
);
...
CREATE TABLE public.workspace_invitations (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT ck_workspace_invitation_status CHECK (((status)::text = ANY ((ARRAY['active'::character varying, 'accepted'::character varying, 'revoked'::character varying, 'expired'::character varying])::text[])))
);
...
CREATE UNIQUE INDEX uq_identity_active_credential_per_purpose ON public.identity_one_time_credentials USING btree (account_id, purpose) WHERE (((status)::text = 'active'::text) AND ((purpose)::text = ANY ((ARRAY['account_activation'::character varying, 'password_recovery'::character varying])::text[])));
